...
class StreamingRelPositionMultiHeadedAttention(MultiHeadedAttention):
    ...
    def forward_parallel_chunk(self, query: torch.Tensor,
        ...
        # Manual replacement for kv.unfold(0, window_size, step_size)
        # kv input shape: (T_kv, H, F)
        # q.shape[1] is query_len_for_chunk (e.g., args.chunk_size)
        query_len_for_chunk = q.shape[1]
        window_size = left_context_size + query_len_for_chunk + right_context_size
        step_size = query_len_for_chunk
        
        T_kv = kv.size(0) # Time dimension of kv after padding

        if T_kv >= window_size:
            num_chunks_unfold = ((T_kv - window_size) // step_size) + 1
            # Pre-allocate output tensor
            kv_out = torch.empty((num_chunks_unfold, kv.size(1), kv.size(2), window_size), dtype=kv.dtype, device=kv.device)
            for i in range(num_chunks_unfold):
                start_idx = i * step_size
                # kv[start_idx:start_idx+window_size, :, :] shape: (window_size, H, F)
                # Chuyển về (H, F, window_size) cho mỗi chunk
                kv_out[i] = kv[start_idx:start_idx+window_size, :, :].permute(1, 2, 0)
            kv = kv_out
        else:
            print(f"Warning: T_kv ({T_kv}) < window_size ({window_size}) in attention manual unfold. Resulting in 0 chunks.")
            kv = torch.empty((0, kv.size(1), kv.size(2), window_size), dtype=kv.dtype, device=kv.device)
        #-----------------------------------------------------------------------------
