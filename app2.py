...
# Đảm bảo thư mục 'chunkformer' chứa các file model và script đã được clone/copy
# Giả định cấu trúc file như gốc của repo Chunkformer:
# chunkformer/
#   model/
#     utils/
#       init_model.py
...
def transcribe_uploaded_file(audio_filepath):
    """
    Handles transcription for an uploaded audio file.
    Includes error handling for file reading, preprocessing, and inference.
    Also adds a specific message if the model returns no recognized text.
    """
    if model is None or char_dict is None:
        print("DEBUG (UPLOAD): Model or char_dict is None at start.")
        return "Lỗi: Hệ thống chưa tải model. Vui lòng kiểm tra logs khi khởi động."

    if audio_filepath is None:
        print("DEBUG (UPLOAD): audio_filepath is None.")
        return "Vui lòng tải lên một file âm thanh."

    if not FFMPEG_AVAILABLE:
        print("DEBUG (UPLOAD): FFmpeg not available.")
        return "Lỗi: FFmpeg/FFprobe không có sẵn. Không thể xử lý file âm thanh. Vui lòng cài đặt FFmpeg."

    audio = None
    max_retries = 3 # Số lần thử lại tối đa
    retry_delay_s = 0.5 # Thời gian chờ giữa các lần thử lại (giây)

    print(f"DEBUG (UPLOAD): Processing uploaded file: {audio_filepath}")
    start_time = time.time()

    # --- Đọc file audio bằng pydub với thử lại ---
    for i in range(max_retries):
        try:
            audio = AudioSegment.from_file(audio_filepath)
            print(f"DEBUG (UPLOAD): Successfully read audio file on attempt {i + 1}.")
            break # Đọc thành công, thoát vòng lặp
        except CouldntDecodeError as e:
            print(f"DEBUG (UPLOAD): Attempt {i + 1}/{max_retries} failed to decode file: {e}")
            if i < max_retries - 1:
                print(f"DEBUG (UPLOAD): Retrying in {retry_delay_s} seconds...")
                time.sleep(retry_delay_s)
            else:
                print(f"DEBUG (UPLOAD): Max retries reached. Could not decode file.")
                return f"Lỗi giải mã file âm thanh sau {max_retries} lần thử. Vui lòng thử lại hoặc sử dụng định dạng file khác. Chi tiết lỗi: {e}"
        except Exception as e:
            print(f"DEBUG (UPLOAD): An unexpected error occurred while trying to read file: {e}")
            return f"Đã xảy ra lỗi không mong muốn khi đọc file: {e}"

    # Nếu audio vẫn là None sau vòng lặp (do tất cả các lần thử đều thất bại), hàm đã trả về lỗi
    if audio is None:
         print("DEBUG (UPLOAD): audio is None after retry loop.")
         return "Lỗi không xác định khi đọc file âm thanh."

    # Initialize default error messages
    preprocess_error_msg = "[Lỗi tiền xử lý hoặc file quá ngắn]"
    inference_error_msg_prefix = "[Lỗi xử lý segment trong inference]" # Match the prefix from transcribe_features

    try:
        original_sr = audio.frame_rate
        original_channels = audio.channels
        print(f"DEBUG (UPLOAD): Original audio: sr={original_sr}, channels={original_channels}, duration_ms={len(audio)}")

        # Chuyển đổi sang định dạng yêu cầu của model (16kHz, mono)
        # Pydub handles resampling and channel conversion
        audio = audio.set_frame_rate(TARGET_SAMPLE_RATE)
        audio = audio.set_channels(1) # Mono
        print(f"DEBUG (UPLOAD): Converted audio: sr={audio.frame_rate}, channels={audio.channels}")

        # Chuyển AudioSegment sang numpy array (float32)
        # Pydub get_array_of_samples returns int16 -> convert to float32 and normalize
        waveform_int16 = np.array(audio.get_array_of_samples(), dtype=np.int16)

        if waveform_int16.size == 0:
            print("DEBUG (UPLOAD): waveform_int16 is empty after conversion.")
            return "Lỗi: File âm thanh trống hoặc quá ngắn sau chuyển đổi."

        # Normalize int16 range [-32768, 32767] to float32 [-1.0, 1.0]
        waveform_float32 = waveform_int16.astype(np.float32) / 32768.0
        print(f"DEBUG (UPLOAD): waveform_float32 created with shape {waveform_float32.shape}")

        # --- Preprocess and extract features ---
        # Pass the actual sample rate from pydub (which should be TARGET_SAMPLE_RATE now) and device
        print(f"DEBUG (UPLOAD): Calling preprocess_numpy_audio with sample_rate={audio.frame_rate} on device {device}")
        feats, x_len = preprocess_numpy_audio(waveform_float32, audio.frame_rate, device)

        raw_text_output = preprocess_error_msg # Assume preprocess error initially if feats is None
        final_display_text = raw_text_output # Assume error initially

        print(f"DEBUG (UPLOAD): preprocess_numpy_audio returned feats: {'None' if feats is None else feats.shape}, x_len: {'None' if x_len is None else x_len.item()}")

        # --- Chạy inference nếu features hợp lệ ---
        inference_attempted = False # Cờ để kiểm tra xem ta có gọi transcribe_features không
        if feats is not None:
            inference_attempted = True
            print("DEBUG (UPLOAD): Calling transcribe_features...")
            raw_text_output = transcribe_features(feats, x_len)
            print(f"DEBUG (UPLOAD): transcribe_features returned raw text: '{raw_text_output}'")

            # --- Xử lý sau khi nhận dạng ---
            # Loại bỏ khoảng trắng thừa và các ký tự không mong muốn
            cleaned_text = re.sub(r'\s+', ' ', raw_text_output).strip()
            print(f"DEBUG (UPLOAD): Cleaned text: '{cleaned_text}'")

            # --- Kiểm tra kết quả nhận dạng ---
            if not cleaned_text: # Nếu văn bản sau làm sạch là rỗng
                 # Check if the raw output wasn't already an error message from transcribe_features
                 # This indicates the model predicted only blank tokens for valid audio features
                 if not raw_text_output.startswith("[Lỗi:"):
                     final_display_text = "Không nhận dạng được lời nói trong file này. (Có thể là im lặng, tiếng ồn, hoặc lời nói không rõ)"
                     print("DEBUG (UPLOAD): Model returned empty output for valid audio features.")
                 else:
                     # If raw_text_output was an error message (e.g., "[Lỗi xử lý segment...]"), keep it
                     final_display_text = raw_text_output
                     print("DEBUG (UPLOAD): transcribe_features returned an error message.")
            else:
                 # Nếu văn bản sau làm sạch KHÔNG rỗng, nhận dạng thành công
                 final_display_text = cleaned_text
                 print("DEBUG (UPLOAD): Recognition successful.")

        else:
             # If feats is None, it means preprocessing failed or audio was too short for features
             print("DEBUG (UPLOAD): Features were not generated (feats is None).")
             # final_display_text is already set to preprocess_error_msg
             pass # Không thay đổi final_display_text, giữ thông báo lỗi tiền xử lý


        end_time = time.time()
        duration = end_time - start_time
        print(f"DEBUG (UPLOAD): Transcription complete for {audio_filepath} in {duration:.2f} seconds. Final text to return: '{final_display_text}'")
        return final_display_text

    except Exception as e:
        print(f"DEBUG (UPLOAD): Error during transcription pipeline (after file read) in upload mode: {e}")
        # Bắt bất kỳ lỗi không mong muốn nào khác trong quá trình xử lý sau khi đọc file
...
if model is not None and char_dict is not None:
    ...
    # Display a simple error message interface if model loading failed
    if not chunkformer_imports_ok:
         error_message = "Gradio interface không khởi chạy do lỗi import các module Chunkformer. Vui lòng kiểm tra PYTHONPATH và cài đặt thư viện."
    else:
         error_message = "Gradio interface không khởi chạy do lỗi tải model. Vui lòng kiểm tra đường dẫn model và các file cần thiết."
    ...
    # Create a simple interface just to show the error message
    error_interface = gr.Interface(
        fn=show_error,
        inputs=[],
        outputs=gr.Text(label="Lỗi Khởi Tạo Hệ Thống"),
        title="Lỗi Khởi Tạo Gradio App",
        description=error_message
    )
    if __name__ == "__main__":
        error_interface.launch(debug=True, share=False)